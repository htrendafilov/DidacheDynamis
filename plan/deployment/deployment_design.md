...
**`publish-image.yml`** (**manual `workflow_dispatch`**, not triggered by a commit):
...
After this, dispatching `publish-image.yml` drives the optional container rollout; commits do not auto-deploy.
...
dispatch `publish-image.yml` → image/DB rebuild and rollout. Native production follows the atomic build/swap
