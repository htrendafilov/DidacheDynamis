...
**No credit display.** `/api/v1/key` returns `limit_remaining`, and two M9.0-era documents referenced a "credit display" here that this list never contained. Decision (2026-07-30): the workspace shows no remaining-credit figure — it invites balance anxiety the app cannot act on, and spend control already lives where it belongs (the dedicated-key spend limit the settings UI tells the user to set). If a future milestone wants one, it is a separate proposal with its own UX rationale; `limit_remaining` is documented as available in `chat/m9.0-findings.md` §11.
